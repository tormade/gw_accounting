...
def test_main_window_does_not_wire_hidden_document_archive_workspace():
    ...
    assert "self.document_archive_panel = DocumentArchivePanel" not in source
    assert "self.document_workspace =" not in source
    assert "tabs.addTab(self.document_archive_panel, \"Archiv\")" not in source
    assert "self.document_archive_panel.document_open_requested.connect" not in source
    assert "self.document_archive_panel.order_open_requested.connect" not in source
